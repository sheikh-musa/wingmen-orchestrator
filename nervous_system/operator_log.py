...
# `finance-console` (2026-08-03, op#9947): the head-of-revenue channel — bot_channels
# mode='agent-session', inject_target='finance', so cc-finance answers it DIRECTLY via
# session injection (verified: it responded to op#9938 on-channel). The hub scope had
# no exclusion for it, so a revenue question the operator asked cc-finance ALSO surfaced
# in the hub's unprocessed() and the hub double-answered it. Neither orch body reconciles
# it — cc-finance does.
_LANE_OWNED_TAGS = ("irsyad-drill", "finance-console")
