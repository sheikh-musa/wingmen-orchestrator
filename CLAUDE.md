...
## Rules
- **Console/Nazim NEVER drives irsyad EXECUTION work (Musa op#21944, 2026-09-22 — hard directive, key separation).** Data applies (migrations/repoints/purges on goumlyne/ceayj), PR merges/rebases, and deploys/re-promotes for irsyad are done by **cc-irsyad-coord (orchestration) + the autoscaler lanes on the musa2 key (execution)** — NOT by this console body (SYED key = NON-irsyad; driving irsyad work from here commingles it onto the wrong key and defeats coord + the autoscaler, which exist precisely to do it). The whole point is key separation. Console's irsyad role is COORDINATION + floor/governance GATES ONLY, and even a gate DECISION is routed to coord to action — the console never executes. **A gate is a yes/no on coord's EVIDENCE PACK — not investigation, design, code review or silo digging (Musa op#22229, 2026-09-24 relapse: I read the code, queried goumlyne and designed the monthly-billing build myself on the Musa key). Missing evidence → send it back to coord; never fetch it yourself.** When an irsyad task lands at this body, delegate it to coord (`_bus_tmp.post('cc-irsyad-coord', …)`); do NOT self-execute, even the money-path applies that the console *technically* holds the write DSN for. (This corrects the prior "console holds the client-silo write path → console executes" pattern; see [[reference_console_holds_client_silo_write_path]] — the write-path capability does NOT make execution the console's job.) **ENFORCED IN CODE (2026-09-24, after the op#22229 relapse):** `scripts/hooks/console_irsyad_guard.py` is a PreToolUse hook (`.claude/settings.json`) that REFUSES any console tool call touching the goumlyne/ceayj silos, the write DSN, or the ihsanos repo/checkouts (keys on the claude process env: `ORCH_BODY_ROLE=console` and no `CC_BASE_AGENT_ID`, so lanes incl. coord and the hub are exempt). Don't work around it; route to coord. Per [[feedback_enforce_process_in_code_not_promises]].
- Never commit `.env`
