...
## 4c. 🔴 GAZZABYTE **DOES** HAVE DIRECT DB ACCESS + 2 APPROVAL-EMAIL PREREQS (op#7157, cai #11302)
Operator asked: *"does gazzabyte even have an account on the irsyad silo?"* **Answer is layered:**
- **APP LAYER: ZERO.** No gazzabyte account in `org_members`; `auth.users` has **no** gazzabyte email at all.
- **INFRASTRUCTURE: YES.** Supabase org `lqbojdqwgzgxhioezfgb` (contains goumlyne) members = `sheikh.musa@outlook.com` **Owner** + **`sales@gazzabyte.sg` Developer** ⇒ **direct dashboard/DB access to the client's live donation data.**
- **⇒ cai's draft line "direct database access, which only we have" is VERIFIED FALSE** (I had flagged it unverifiable at v2). **Must come out of v4** — a wrong claim about who can reach their data, made *to the party that can reach it*. Also quietly propped up the **CAI-561 P1 reachability grading** (the anon-key reasoning stands; the "only we" clause does not).
- **⚠️ ONLY THREE REAL PEOPLE on the irsyad org:** Elly (preparer), Saddam (org_admin), **Zuremi (org_admin — `accepted_at IS NULL`, NEVER accepted her invite)**.
- **🔴 APPROVAL-EMAIL PREREQ (a): Zuremi has never completed signup** and is one of the two intended approvers ⇒ emails to her are undeliverable in practice; may explain her silence on the outstanding client questions.
- **🔴 APPROVAL-EMAIL PREREQ (b): NINE test accounts live on the client org, FOUR holding `org_admin`** — admin@irsyad.test · uat-operator@qa-madrasah.test · zz-uat-tester@irsyad-uat.test (org_admin); cashier@irsyad.test · uat-cashier; viewer@irsyad.test · uat-viewer; uat-teacher; parent@irsyad.test. **Not a send-risk today** (cc-reviewer confirmed recipients resolve from the editable allowlist, never the org_admin roster, failing closed to `[]`) — but exactly what a September auditor finds, and one permissive change from mattering. **Removal recommended → client-silo mutation → needs cai grant. NOT touched.**
- **✅ mig 120 FREEZE CONDITION NOW MET** — CAI-548 bound *"neither migration applies until the renumber lands"*; the renumber landed and merged (`c329c19`). `120_tabung_approval_notifications.sql` is eligible for a §6.6 grant whenever cai is back.
- **GIRO told to operator honestly as FURTHER OUT** — synthetic prototype, never run on real data; needs the audit prerequisites (its commit path writes the same audit rows), cai's 4 CAI-529 binds, and 3 unanswered client questions (retention, ±3-day tolerance, period sign-off). **No dates given for either item.**

## 4d. ✅ TEST-ACCOUNT CLEANUP DONE (op#7159) · ⏳ GAZZABYTE ADMIN NOT CREATED
**DONE — 9 test-account memberships revoked** on goumlyne org 73339164 (`org_members.deleted_at=now()`): admin@irsyad.test · uat-operator@qa-madrasah.test · zz-uat-tester (org_admin) · cashier@irsyad.test · uat-cashier · viewer@irsyad.test · uat-viewer · uat-teacher · parent@irsyad.test. **Active members now exactly: Elly (preparer), Saddam (org_admin), Zuremi (org_admin, unaccepted).** Guarded by assertions (target==9, keep-set=={elly,saddam,zuremi}) so a wrong email list would abort rather than proceed.
- **⚠️ DID NOT delete `auth.users` rows — deliberately.** `admin@irsyad.test` is `actor_id` on **642 audit rows incl. the historical tabung load**; uat-operator on 18 more. Deleting would **orphan the audit trail we're mid-correction about**, days before the client's audit. Membership revocation removes all access and preserves history. *(If full deletion is ever wanted, that trade-off must be on the table explicitly.)*
- **⚠️ DID NOT hand-write audit rows** — `audit_log` is a hash chain; a manual INSERT breaks linkage, the one property still holding across all 676 rows and the one the client correction leans on. **Chain re-verified after: still 676.**
- Disclosed to cai (#…, P2) with the reasoning for proceeding while he's P0-only: access-**reducing**, reversible, operator-directed, no money data, and **no active cai gate on org membership** (his open gates = bank-import commit, mig 119/120/121, GIRO). **Asked him to say if org-membership changes are inside the gate regardless of direction.**
- **⏳ NOT DONE — Gazzabyte org_admin account** (operator asked, so they can use view-as). **Access-WIDENING to an external party ⇒ cai's gate.** Went back to the operator for a **named person vs the shared `sales@gazzabyte.sg` mailbox** (admin actions on a live donation system attributed to an inbox weakens the very trail we're disclosing limits on), and insisted it go through **the app invite flow, not a hand-crafted DB row** (a raw insert grants access with no audit row and no working login). Honest counter-argument given: Gazzabyte **already** holds Supabase **Developer** on the org, so an audited app admin is arguably a *tightening*.
