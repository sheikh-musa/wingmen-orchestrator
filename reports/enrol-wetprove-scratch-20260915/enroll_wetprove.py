...
Proves the enroll match-and-enrich migration (mig337, commit cee10763; op#39942 /
...
THE LOCKED CONTRACT (mig337, verified at source cee10763 — see RPC_CONTRACT below)
---------------------------------------------------------------------------------
...
HOW TO RUN (coord/console, per silo — the builder has NO write DSN):
  ...
  columns + base fn, then the REAL mig337 bytes from cee10763 which CoR-replace the
...
# -----------------------------------------------------------------------------
# Bound to mig337 (cee10763), verified at source. If the migration is renumbered,
...
try:
    ...
    P3, S3 = make_name_only("Cahaya Test", "STU-103", CLS_1A,         # never-clobber target
                            ...
                            stu_extra={"house": "ORIGINAL-HOUSE"})
    ...
    full_fields = {
        ...
        "house": "GREEN", "citizenship": "SG", "nationality": "Singaporean",
    ...
    ck("[2 NEVER-CLOBBER] S3 pre-existing persons.gender='male' NOT overwritten by file 'female'",
       s3p["gender"] == "male")
    ck("[2 NEVER-CLOBBER] S3 pre-existing sch_students.house='ORIGINAL-HOUSE' NOT overwritten",
       s3s["house"] == "ORIGINAL-HOUSE")
    reset_role()
    ...
    # 4e: audit hash-chain intact + appended after revert.
    cur.execute("set local role service_role")
    cur.execute("select prev_hash, hash, payload::text from audit_log where org_id=%s order by id asc",
                (ORG,))
    rows = cur.fetchall()
    cur.execute("select count(*) from audit_log where org_id=%s", (ORG,))
    ...
    for prev_hash, h, payload in rows:
        cur.execute("select encode(sha256(convert_to(%s || %s, 'UTF8')),'hex')", (prev_hash, payload))
        (recomputed,) = cur.fetchone()
        if h != recomputed or (prev is not None and prev_hash != prev):
            chain_ok = False
            break
        prev = h
    ck("[4e AUDIT] hash-chain intact after revert (every hash == sha256(prev||payload), links unbroken)",
