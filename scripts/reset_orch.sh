...
if [ "$CC_EMPTY" != 1 ]; then
  ...
  if [ "$CC_FLAT" = "$CC_BEFORE_WIPE" ]; then
    echo "[reset_orch] GHOST: composer byte-identical after ${WIPE} BSpace ('${CC_FLAT}') — real text cannot survive that, so it is EMPTY underneath. Proceeding (probe-confirmed, no force needed)." >&2
  else
