#!/usr/bin/env bash
# cosem_tdu_support_send.sh — send a message to the cosem-tdu support channel,
# sibling to scripts/irsyad_support_send.sh (op#22426).
#
# Supervised shape: a reviewer (cc-cosem-tdu-coord, per bot_channels.group_routing
# on channel_key='cosem-tdu') sends here — the raw builder lane never sends
# directly. Reads COSEM_TDU_BOT_TOKEN from .env. NEVER echoes the token.
#
# NOT gated by console_irsyad_client_send_gate.sh (bus cce66db2-...#43311): that
# gate is op#21944's irsyad-specific console/execution key-separation control and
# does not extend to cosem-tdu. Generic anti-leak / arg-swap guards still apply.
#
# Usage:
#   scripts/cosem_tdu_support_send.sh "message text"
#   echo "message" | scripts/cosem_tdu_support_send.sh
#   TG_CHAT_OVERRIDE=<chat_id> scripts/cosem_tdu_support_send.sh "message text"
set -euo pipefail
...
[ "$sent" = 1 ] && exit 0 || { echo "cosem_tdu_support_send failed" >&2; exit 1; }
