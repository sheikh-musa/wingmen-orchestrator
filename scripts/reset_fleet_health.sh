...
if [ "$CC_EMPTY" != 1 ] && [ "${CC_GHOST:-0}" != 1 ]; then
  echo "ERROR: composer NOT empty after wipe — refusing to send /clear into dirty input (residue: ${CC_FLAT}). SRE UNCHANGED." >&2
  exit 6
fi
...
BOOT="You are cc-fleet-health, the fleet SRE (fleet reliability / health), singleton, freshly reset in-place by ${RESET_BY:-orch-console/Nazim} at $(date -u +%Y-%m-%dT%H:%MZ) because you were running degraded (82% ctx) + idling — a bloated body comes back fresh rather than sitting stuck. THIS BOOT MESSAGE MAKES NO CLAIM about your model or your agenda — it is a fixed string that goes stale. ${HANDOFF} IS THE AUTHORITY: read it IN FULL FIRST, then $FH_DIR/CLAUDE.md (your charter), and where they disagree the handoff wins. TAKE/RENEW the fleet_health_lease per your boot sequence. Reconcile agent_messages where to_agent='cc-fleet-health' and read_at is null; stamp what you process. ${STAGED_NOTE} STANDING: verify-not-assert (a name is not an implementation; a failed measurement reports 'could not measure', never a finding); when a premise falls, RE-DERIVE. Your immediate committed work per the handoff: finish the agent_wake verified-submit fix (port lane_nudge's clear→type→Enter→retry), and note the independent self-liveness watchdog (dev.wingmen.sre-liveness, orch-console-built, you reviewed+approved) is now live as your safety-net. Reply to Nazim (agent_messages to 'orch-console') once you are up + confirm fresh ctx."
