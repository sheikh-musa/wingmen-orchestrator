-- CAI-RESP-163 Q3: active_autonomous_loops.parent_pid column + boot_briefing surface
--
-- Adds parent_pid to active_autonomous_loops. The detector
-- (nervous_system/autonomous_loop_detector.py) populates this column via
-- `lsof -d cwd` + cwd-match against the per-cc-identity repo path. The Phase B
-- watchdog (long_caller_watchdog) consumes this row at kill time and
-- re-verifies the PID's cwd before SIGTERM to guard against PID recycle
-- (CAI-RESP-163 Q3 PID-recycle race).
--
...
-- ============================================================================
-- Section 1: add parent_pid column (nullable; detector populates lazily)
-- ============================================================================
...
COMMENT ON COLUMN active_autonomous_loops.parent_pid IS
    'PID of the long-running claude process whose cwd matches the cc_identity''s '
    'repo path. Populated by nervous_system/autonomous_loop_detector.py via '
    '`lsof -d cwd` + cwd-match (CAI-RESP-163 Q3). The Phase B long_caller_watchdog '
    're-verifies the PID''s cwd at kill time to guard against PID recycle between '
    'detector sweep and SIGTERM.';

-- ============================================================================
-- Section 2: extend boot_briefing — only the active_autonomous_loops arm
--            every other arm preserved verbatim from prior view definition
-- ============================================================================
...
BEGIN
    SELECT EXISTS (
        ...
        WHERE table_name = 'active_autonomous_loops'
          AND column_name = 'parent_pid'
    ) INTO col_exists;
    IF NOT col_exists THEN
        RAISE EXCEPTION 'active_autonomous_loops.parent_pid column missing after ADD COLUMN';
    END IF;

    SELECT pg_get_viewdef('boot_briefing'::regclass, true) INTO view_def;
    IF position('parent_pid' IN view_def) = 0 THEN
        RAISE EXCEPTION 'boot_briefing view definition missing parent_pid after CREATE OR REPLACE';
    END IF;
    RAISE NOTICE 'active_autonomous_loops.parent_pid + boot_briefing arm verified';
END $$;
