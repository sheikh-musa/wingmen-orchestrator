...
| `SHARE_USER` / `SHARE_PASSWORD` (share.wingmen.dev) | HTTP Basic Auth on ALL client share docs (r/gazzabyte-status etc.) — **rotation-recoverable**, low-sensitivity (deliberately client-shared; no money/PII behind the gate) | **SOURCE OF TRUTH = `wingmen-share` Vercel prod env** (`vercel env pull`; user=`wingmen`) | none needed (rotation-recoverable) | **VERIFIED 2026-08-14** via live-URL fetch (HTTP 200). ⚠ **The local `~/wingmen/wingmen-share/.share-password` file is STALE/DRIFTED — do NOT trust it** (it 401'd against the live gate). ALWAYS pull the pw from Vercel env, never that file. Single shared cred across all share docs; guards the Gazzabyte status page delivered to Shuk. |
