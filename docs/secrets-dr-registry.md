...
| `NRIC_ENCRYPTION_KEY` (ceayj / ihsanos) | AES-GCM master key — ceayj NRIC/phone/email (66 real rows) | ceayj (`ihsanos`) Vercel project env | TBD | **IN PROGRESS** — CAI-848. cc-ihsanos checking Vercel-sensitivity → if readable, #6692-style no-echo pull to a 0600 file (verify-decrypts-66, 2 copies); if sensitive/write-only, operator-manual |
...
- Close the ceayj `NRIC_ENCRYPTION_KEY` backup (CAI-848 ceayj leg).
