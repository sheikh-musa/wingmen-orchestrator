...
6. **SELECT-visibility is a hard prerequisite for the per-agent rollout.** With RLS on and only an INSERT `WITH CHECK` policy, a non-`BYPASSRLS` role can INSERT but cannot read back any row (incl. its own inbox) or `INSERT … RETURNING` (RETURNING is filtered by SELECT policies → raises "violates RLS"). The legacy `service_role` path is unaffected (it bypasses RLS).
   ...
   - **`strategic_decisions` SELECT visibility — OPEN FORK, escalated to cai.** Decisions are a shared governance ledger but per-agent roles do not currently boot from it directly (the orchestrator reads it via `service_role`). Options: (a) own-only (`decided_by = identity OR posted_by_identity = identity`) — least privilege, supports `RETURNING`; (b) shared-ledger (all agents read all decisions) — matches boot_briefing's shared-context model. Recommend (a) now, broaden to (b) only when a per-agent boot path needs it. Awaiting cai's call before adding the `strategic_decisions` SELECT policy.
