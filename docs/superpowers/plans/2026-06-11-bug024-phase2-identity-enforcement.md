...
6. **This migration adds NO SELECT policy — and per-agent roles will need one before they go live (operator follow-up, gating).** With RLS enabled and only an INSERT `WITH CHECK` policy, a non-`BYPASSRLS` role can INSERT but (a) cannot `INSERT … RETURNING` (RETURNING is filtered by SELECT policies, of which there are none → it raises "new row violates row-level security policy"), and (b) cannot read back any row, including its own inbox. The legacy `service_role` path is unaffected (it bypasses RLS). The correct SELECT-visibility model (which agent may read which messages) is a separate design decision and is deliberately **out of scope** here — but it is a hard prerequisite for the operator-gated per-agent credential rollout: provisioning the roles without SELECT policies would break every agent read and any `RETURNING`-based insert. Surfaced to cai/operator as a required Track-A companion. (Tests insert without `RETURNING` and verify via a superuser read-back to stay within this migration's INSERT-only scope.)
