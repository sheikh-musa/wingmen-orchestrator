...
**Status:** Approved (Musa scope A; CAI-RESP-036 + CAI-RESP-042 binding)
**Parent decisions:** ARCH-035 (decision_ref), CAI-RESP-036 (8-item amendment), CAI-RESP-042 (3-item pre-write clarification)
...
CREATE TABLE agent_status (
  ...
  status                  TEXT NOT NULL CHECK (status IN ('idle','working','blocked','reviewing','offline')),
  current_task            TEXT,
  scope_repos             TEXT[],
  scope_paths             TEXT[],
...
-- 4. Identity tripwire (B2) — SECURITY DEFINER trigger compares agent_id to GUC
...
- 90-day TTL cron on `agent_status_history` deferred to follow-up TASK.
...
1. Sets `app.current_agent_id` GUC to `$AGENT_ID` for the connection
2. UPSERTs `agent_status` with `agent_id=$AGENT_ID, status='working', current_task='session-launch'`

**Open implementation detail (subagent's first job to confirm):** PostgREST may not propagate session GUC across pooled connections. If that's the case, the migration grows by one RPC wrapper:

```sql
CREATE OR REPLACE FUNCTION set_agent_id_and_upsert_status(
  p_agent_id TEXT,
  p_status TEXT,
  p_current_task TEXT
) RETURNS VOID AS $$
BEGIN
  PERFORM set_config('app.current_agent_id', p_agent_id, true);
  INSERT INTO agent_status (agent_id, status, current_task, last_heartbeat)
  VALUES (p_agent_id, p_status, p_current_task, now())
...
Subagent reports back; we decide whether RPC route is needed before commit. If yes, RPC is added to the migration in the same commit.
...
1. **PostgREST GUC propagation** (Step 4 open detail) — if `SET LOCAL` does not survive the supabase-py call boundary, fallback is the RPC wrapper documented above. Subagent confirms before commit; if RPC is needed, it joins the migration in the same commit.
2. **Subprocess Python in launch script adds boot latency** (~200-500 ms). Acceptable; alternative is a separate post-launch Python hook, which loses atomicity. Going with in-script for atomicity.
3. **Banned-prefix tripwire keeps offending messages UNREAD forever** until manually resolved. Intentional (A1 — discipline visible). Can revisit if it becomes noisy.
...
- `TASK-NNN` (file post-ship): 90-day TTL cron for `agent_status_history` (per CAI-RESP-036 B3).
