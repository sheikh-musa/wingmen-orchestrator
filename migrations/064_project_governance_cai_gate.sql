...
COMMENT ON FUNCTION public.project_for_repos(TEXT[]) IS
  'Resolve a strategic_decisions.repos_affected array to its governed project by '
  'ILIKE substring match against any element (project_governance_families, '
  'match_type=repo_pattern). NULL = no matching family -- passes ungoverned.';
...
-- decision whose resolved project has cai_enabled=false. Unresolvable /
-- non-cai-decided rows pass untouched.
-- ---------------------------------------------------------------------------
...
DECLARE
  p TEXT;
BEGIN
  IF NEW.decided_by = 'cai' THEN
    p := public.project_for_repos(NEW.repos_affected);
    IF p IS NOT NULL AND EXISTS (
      SELECT 1 FROM public.project_governance g
       WHERE g.project = p AND g.cai_enabled = false
    ) THEN
      RAISE EXCEPTION USING
        ERRCODE = 'check_violation',
        MESSAGE = format(
          'cai is OFF for project %s (op#20708): cai cannot bind this decision '
          '-- route to Musa / the project operator instead', p
        );
    END IF;
  END IF;
  RETURN NEW;
