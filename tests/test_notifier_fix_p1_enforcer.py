...
def test_enforcer_test_mode_true_full_write_path():
    ...
    with psycopg.connect(_dsn(), autocommit=True) as conn, conn.cursor() as cur:
        # Save current mode to restore after test
        cur.execute("SELECT value FROM orchestrator_runtime_config WHERE key = 'challenge_enforcer_mode'")
        original_mode = cur.fetchone()[0]
        # Flip to write_mode — SAFE because test_mode=TRUE predicate excludes is_test=FALSE rows
        ...
        try:
            ...
            try:
                ...
                cur.execute("DELETE FROM strategic_decisions WHERE decision_ref = 'TEST-ENFORCE-TESTMODE-WRITE'")
        finally:
            # Restore enforcer_mode
            cur.execute("UPDATE orchestrator_runtime_config SET value = %s WHERE key = 'challenge_enforcer_mode'", (original_mode,))
